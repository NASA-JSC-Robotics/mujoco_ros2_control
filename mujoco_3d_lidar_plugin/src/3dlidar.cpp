...
Lidar *Lidar::Create(const mjModel *m, mjData *d, int instance) {
  ...
  std::string res_str =
      std::string(mj_getPluginConfig(m, instance, "resolution"));
  ...
  std::string azimuth_range_str =
      std::string(mj_getPluginConfig(m, instance, "azimuth_range"));
  ...
  std::string elevation_range_str =
      std::string(mj_getPluginConfig(m, instance, "elevation_range"));
  ...
  if ((resolution[1] > 1) && (elevation_range[0] >= elevation_range[1])) {
    mju_error(
        "`elevation_range minimum` must less than 'elevation_range maximum");
    return nullptr;
  }

  std::string max_range_str =
      std::string(mj_getPluginConfig(m, instance, "max_range"));
  ...
  return new Lidar(m, d, instance, resolution.data(), azimuth_range.data(),
                   elevation_range.data(), max_range);
}

Lidar::Lidar(const mjModel *m, mjData *d, int instance, int resolution[2],
             mjtNum azimuth_range[2], mjtNum elevation_range[2],
             mjtNum max_range)
  ...
  if (resolution_[1] > 1) {
    LinSpace(elevation_range[0], elevation_range[1], resolution_[1],
             elevationAngles);
  } else {
...
void Lidar::Compute(const mjModel *m, mjData *d, int instance) {
  ...
  for (int idx = 0; idx < dim; ++idx) {
    mjtNum vec[] = {vectors_[idx * 3], vectors_[idx * 3 + 1],
                    vectors_[idx * 3 + 2]};
  ...
  int *geomid = nullptr;
  mj_multiRay(m, d, site_pos, rotated_vectors_.data(), geom_group, flg_static,
              body_exclude, geomid, sensordata, NULL,
              resolution_[0] * resolution_[1], max_range_);
#elif (mjVERSION_HEADER == 340)
  std::vector<int> geomid(resolution_[0] * resolution_[1]);
  mj_multiRay(m, d, site_pos, rotated_vectors_.data(), geom_group, flg_static,
              body_exclude, geomid.data(), sensordata,
              resolution_[0] * resolution_[1], max_range_);
#else
...
void Lidar::Visualize(const mjModel *m, mjData *d, const mjvOption *opt,
    ...
    if (rotated_vectors_.size() == dim * 3) {
      for (int idx = 0; idx < dim; ++idx) {
        ...
        if (dist >= 0) {
          ...
          mjv_initGeom(thisgeom, mjGEOM_LINE, nullptr, nullptr, nullptr,
                       m->vis.rgba.rangefinder);
...
void Lidar::RegisterPlugin() {
  ...
  const char *attributes[] = {"resolution", "azimuth_range", "elevation_range",
                              "max_range"};
